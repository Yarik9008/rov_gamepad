...
Serial2 (PA2/PA3)  → отладочный вывод (115200 baud)
```
...
struct ROVCommands {
    ...
    int light;             // Уровень освещения (0-255)
};
```
...
void update() {
    ...
    updateManipulator(joy_data);  // Управление манипулятором
    updateLighting(joy_data);     // Управление освещением
...
- **Тип:** Мгновенное переключение
- **Состояния:**
  - Выключено: LIGHT_OFF (0)
  - Включено: LIGHT_ON (255)
...
struct JoystickCoefficients {
    float stick1_x = 1.0;    // Джойстик 1 - X ось (вперед/назад)
    float stick1_y = 1.0;    // Джойстик 1 - Y ось (поворот)
    float stick2_x = 1.0;    // Джойстик 2 - X ось (вертикальное движение)
...
struct KinematicsConfig {
    int motor_count = 4;                    // Количество моторов
...
- **Интервал:** DATA_INTERVAL мс между командами
...
- **Частота обновления:** ~100 Гц (10 мс интервал)
...
### Индикация состояния
- **LED (PC13):** Мигает каждые LED_INTERVAL мс
