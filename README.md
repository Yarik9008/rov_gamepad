...
#### Настройка инверсии камеры

**Когда нужна инверсия камеры?**
- Камера поворачивается в противоположную сторону от ожидаемой
- Сервопривод установлен "вверх ногами" или повернут
- Неправильное подключение сервопривода камеры
- Нужно изменить направление без изменения подключения

**Как настроить инверсию камеры:**
1. **Откройте файл** `src/Config.h`
2. **Найдите строку** `#define CAMERA_INVERT false`
3. **Измените значение:**
   - `false` - нормальное направление (по умолчанию)
   - `true` - инвертированное направление
4. **Перекомпилируйте** проект: `pio run`
5. **Загрузите** новую прошивку в микроконтроллер
6. **Протестируйте** поворот камеры

**Примеры настройки:**
```cpp
// Нормальное направление (по умолчанию)
#define CAMERA_INVERT false
// BUTTON0 → вниз, BUTTON1 → вверх

// Инвертированное направление
#define CAMERA_INVERT true  
// BUTTON0 → вверх, BUTTON1 → вниз
```
